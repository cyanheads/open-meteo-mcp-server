...
- Coverage gap (regional model outside its domain), either shape upstream reports it in — `nan` coordinates in an unparseable HTTP 200 body, or a 4xx `No data is available for this location` envelope → throw the same `ValidationError`, non-retryable
