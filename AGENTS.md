...
This server declares `sessionMode: 'stateless'` — no tool calls `ctx.requestInput` and no state is held per session. `MCP_SESSION_MODE` remains an override.
...
src/
  ...
  services/
    ...
    canvas-accessor.ts                  # DataCanvas accessor for the spill-capable tools (forecast / historical / marine / air quality / ensemble / flood / climate)
  mcp-server/
    tools/definitions/
      ...
      get-marine.tool.ts                # openmeteo_get_marine (DataCanvas spillover)
      get-air-quality.tool.ts           # openmeteo_get_air_quality (DataCanvas spillover)
...
| `bun run release:github` | Create a GitHub Release from the latest tag — attaches `.mcpb` bundle if present |
